...
#define MARK_BIT (1UL << 1)

static inline bool is_marked(const Block *block);
static inline void set_mark(Block *block, bool value);
static Block* find_containing_block(const void* ptr);
...
static Block* find_containing_block(const void* ptr) {
  // From a given pointer, find its containing block
  // we do a simple linear scan 

  if ((char*) ptr < (char*) gFirstChunk || (char*) ptr > (char*) gLastChunk) {
    return NULL;
  }

  for (Chunk* chunk = (Chunk*) gFirstChunk; chunk; chunk = chunk->next) {
    const char* chunk_start = chunk;
    const char* chunk_end = chunk + chunk->size;

    // check if the pointer is between the curr chunk
    if ((char*) ptr >= chunk_start && (char*) ptr <= chunk_end) {
      // linear scan
      Block* curr_block = find_first_block_in_chunk(chunk);
      while (curr_block && curr_block->chunk == chunk) {
        // check if the pointer is between block 
        const char* block_start = (char*) curr_block;
        const char* block_end = block_start + block_size(curr_block);
...
// recursively mark blocks
static void mark_block(Block* block) {
  if (!block) return;
  if (is_marked(block)) return; // prevent cyclic 
  if (is_free(block)) return; // no need to mark free blocks 

  set_mark(block, true);

  char* data_start = (char*) block + kMetadataSize;
  char* data_end = data_start + block_size(block) - kBoundarySize;
  
  // why we stop at p + sizeof(char*) <= data_end?
  // because we want to read the last pointer at data_end - sizeof(char*)
  // if it reads data_end by sizeof(char*) then we will read more than this block
  for (char* p = data_start; p + sizeof(char*) <= data_end; p += sizeof(char*)) {
    void* candidate = *(void**)p;
    Block* maybe_block = find_containing_block(candidate);

    if (maybe_block && !is_marked(maybe_block) && !is_free(maybe_block)) {
      mark_block(maybe_block);
    }
  }
}

static void clear_all_marks() {
  for (Chunk* curr_chunk = gFirstChunk; curr_chunk; curr_chunk = curr_chunk->next){
    Block* curr_block = find_first_block_in_chunk(curr_chunk);

    while (curr_block && curr_block->chunk == curr_chunk) {
      set_mark(curr_block, false);
      curr_block = get_next_block(curr_block);
    }
  }
}
...
static void mark_phase() {
  ...
  // first we clear all marks first
  clear_all_marks();

  // (2) we determine the stack range
  void* stack_ptr = get_end_of_stack();
  char *low, *high;
  if (start_of_stack < stack_ptr) {
    low  = (char*)start_of_stack;
    high = (char*)stack_ptr;
  } else {
    low  = (char*)stack_ptr;
    high = (char*)start_of_stack;
  }

  // scan from start to end
  for (char** scan = (char**) low; scan < (char**) high; scan++) {
    void* possible_ptr = *scan;
    Block* b = find_containing_block(possible_ptr);
    if (b) {
      mark_block(b);
    }
  }
...
static void sweep_phase() {
  for (Chunk* curr_chunk = (Chunk*) gFirstChunk; curr_chunk; curr_chunk = curr_chunk->next) {
    Block* curr_block = find_first_block_in_chunk(curr_chunk);
