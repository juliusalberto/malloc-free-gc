...
def run_tests(tests_path: Path, internal_tests_path: Path, script_path: Path):
    for directory in [tests_path, internal_tests_path]:
        for file in os.listdir(directory):
            file, ext = os.path.splitext(directory / file)
...
def main():
    ...
    if args.test:
        test_dir = ""
        if (os.path.exists(f"./tests/{args.test}.c")):
            test_dir = "tests"
        elif (os.path.exists(f"./internal-tests/{args.test}.c")):
            test_dir = "internal-tests"
        else:
            raise Exception(f"{bcolors.FAIL}Cannot find test name {args.test}")
        output, exit_code = make(f"{test_dir}/{args.test} " + build_cmd, script_path)
        check_make(f"{test_dir}/{args.test}", output, exit_code)

        test_path = script_path / test_dir / args.test

        output, exit_code = run_test(test_path, script_path)
        check_test(test_path, output, exit_code, script_path)
    else:
        output, exit_code = make("test internal " + build_cmd, script_path)
        check_make("test", output, exit_code)

        run_tests(script_path / "tests", script_path / "internal-tests", script_path)
